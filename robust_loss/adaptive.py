...
from robust_loss import distribution, util, wavelet
...
def _check_scale(scale_lo, scale_init):
    ...
    if not np.isscalar(scale_init):
        raise ValueError(
            "`scale_init` must be a scalar, but is of type {}".format(type(scale_init))
        )
    if not scale_lo > 0:
        raise ValueError("`scale_lo` must be > 0, but is {}".format(scale_lo))
    if not scale_init >= scale_lo:
        raise ValueError(
            "`scale_init` = {} must be >= `scale_lo` = {}".format(scale_init, scale_lo)
        )
...
def _construct_scale(x, scale_lo, scale_init, float_dtype):
    ...
    if scale_lo == scale_init:
        ...
        scale = tf.tile(
            tf.cast(scale_init, float_dtype)[tf.newaxis, tf.newaxis], (1, x.shape[1])
        )
    else:
    ...
    def __init__(
        ...
        if alpha_lo != alpha_hi:
            ...
            self._latent_alpha = tf.Variable(
                tf.fill(
                    (1, num_channels), tf.cast(latent_alpha_init, dtype=float_dtype)
                ),
                name="LatentAlpha",
            )
...
class StudentsTLossFunction(tf.Module):
    ...
    def __init__(
        self, num_channels, float_dtype, scale_lo=1e-5, scale_init=1.0, name=None
    ):
        ...
        with tf.control_dependencies(
            ...
            if self._representation in wavelet.generate_filters():
                x_stack = wavelet.flatten(
                    wavelet.rescale(
                        wavelet.construct(
                            x_stack, self._wavelet_num_levels, self._representation
                        ),
